...
def test_get_authors(client, sample_data):
    ...
    assert response_data['numbers of records'] == 5
...
def test_get_authors_with_params(client, sample_data):
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert response_data['success'] is True
    assert response_data['numbers of records'] == 2
    assert response_data['pagination'] == {
        'total_pages': 5,
        'total_records': 10,
...
def test_create_author(client, token, author):
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert response_data == expecter_result
...
@pytest.mark.parametrize(
    "data,missing_fields",
    [
        ({'last_name': "Mickiewicz", 'birth_date': '21-06-1948'}, 'first_name'),
        ({'first_name': "Adam", 'birth_date': '21-06-1948'}, 'last_name'),
        ({'last_name': "Mickiewicz", 'first_name': "Adam"}, 'birth_date'),
    ]
)
def test_create_author_invalid_data(client, token, data, missing_fields):
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert response_data['success'] is False
    assert 'data' is not response_data
    assert missing_fields in response_data['message']
    assert "Missing data for required field." in response_data['message'][missing_fields]


def test_create_author_invalid_content_type(client, token, author):
    response = client.post('/api/v1/authors',
                           data=author,  headers = {"Authorization": f"Bearer {token}"
                        })
...
def test_create_author_invalid_missing_token(client, author):
    response = client.post('/api/v1/authors',
                           json=author,  )
