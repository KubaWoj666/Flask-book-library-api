...
class ErrorResponse:
    ...
    def to_response(self) -> Response:
        response = jsonify(self.pyload)
        response.status_code = self.http_status
        return response


# po wprowadzeniu Blueprint zmieniamy dekorator @app na errors_bp
# i errorhandler na app_errorhandler
...
def internal_server_error(err):
    # rollback() metoda która czyści sejse w bazie danych
    db.session.rollback()
    return ErrorResponse(err.description, 415).to_response()
